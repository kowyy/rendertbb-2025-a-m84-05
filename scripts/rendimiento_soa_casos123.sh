#!/bin/bash
# rendimiento_soa.sh - Mide rendimiento de soa
...
BINARY="out/build/default/soa/Release/render-soa"
...
echo "========================================="
echo " EVALUACIÓN DE RENDIMIENTO soa"
echo "========================================="
echo ""
...
perf stat -r 15 -a -e cycles,instructions,power/energy-pkg/,power/energy-ram/ \
    "$BINARY" "$CONFIG_DIR/config1.txt" "$CONFIG_DIR/scene1.txt" "$OUTPUT_DIR/soa_1.ppm" \
    2>&1 | tee "$OUTPUT_DIR/perf_soa_caso1.log"
...
perf stat -r 10 -a -e cycles,instructions,power/energy-pkg/,power/energy-ram/ \
    "$BINARY" "$CONFIG_DIR/config2.txt" "$CONFIG_DIR/scene2.txt" "$OUTPUT_DIR/soa_2.ppm" \
    2>&1 | tee "$OUTPUT_DIR/perf_soa_caso2.log"
...
perf stat -r 7 -a -e cycles,instructions,power/energy-pkg/,power/energy-ram/ \
    "$BINARY" "$CONFIG_DIR/config3.txt" "$CONFIG_DIR/scene3.txt" "$OUTPUT_DIR/soa_3.ppm" \
    2>&1 | tee "$OUTPUT_DIR/perf_soa_caso3.log"
